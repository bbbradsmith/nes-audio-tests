;
; nsf2_init_play.s
;   verification of NSF2 non-returning INIT and NMI-drivern PLAY feature
;
;   1. INIT should be called once and let return.
;   2. After INIT first returns, PLAY will be enabled.
;   3. After enabling PLAY, INIT will be called again and is allowed to enter an infinite loop.
;   4. PLAY will interrupt the non-returned INIT loop at its specified rate.
;
;   Verify:
;     When INIT is called a second time, it plays a 250Hz 0-duty square wave tone.
;     When PLAY is called regularly it plays a 100Hz square wave through $4011.
;     The result should hear both of these played together.
;
...
text_chunk:
.define NL 13,10
.byte ";   verification of NSF2 non-returning INIT and NMI-drivern PLAY feature",NL
.byte ";",NL
.byte ";   1. INIT should be called once and let return.",NL
.byte ";   2. After INIT first returns, PLAY will be enabled.",NL
.byte ";   3. After enabling PLAY, INIT will be called again and is allowed to enter an infinite loop.",NL
.byte ";   4. PLAY will interrupt the non-returned INIT loop at its specified rate.",NL
.byte ";",NL
.byte ";   Verify:",NL
.byte ";     When INIT is called a second time, it plays a 250Hz 0-duty square wave tone.",NL
.byte ";     When PLAY is called regularly it plays a 100Hz square wave through $4011.",NL
.byte ";     The result should hear both of these played together.",NL
.byte ";",NL
